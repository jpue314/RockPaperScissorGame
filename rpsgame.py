...
def show_leaderboard():
    ...
    sorted_leaders.sort(key=lambda x: x[1], reverse=True)
...
def play_game(player_1, player_2):
    log(f"New game starting between {player_1} vs {player_2}.")
    ...
    while not find_winner(wins, wins.keys()):
        ...
        log(f"{player_1} roll {roll1} and {player_2} roll {roll2}.")
...
def log(msg):
    directory = os.path.dirname(__file__)
    filename = os.path.join(directory, "rps.json")
    ...
    def get_completions(self, document, complete_event):
        ...
        for roll in roll_names:
            ...
            if complete_all or is_substring:
                completion = completions.append(Completion(roll,
